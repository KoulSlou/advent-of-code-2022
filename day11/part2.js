...
for (let i = 0; i < 10000; i++) {
  ...
  for (monkey of monkeys) {
    ...
    for (item of monkey.items) {
      ...
      let transformedWorryLevel = newWorryLevel % magicNumber;
      //   console.log(newWorryLevel);
      monkey.inspectorCounter++;

      //   console.log(worryLevelAfterInspection);
      if (monkey.test(transformedWorryLevel)) {
        // console.log("throw to " + monkey.onPassTo);
        monkeys[monkey.onPassTo].addItem(transformedWorryLevel);
      } else {
        // console.log("throw to " + monkey.onFailTo);
        monkeys[monkey.onFailTo].addItem(transformedWorryLevel);
      }
      monkey.items = [];
    }
  }
